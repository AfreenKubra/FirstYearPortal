...
/**
 * Only PDF and CSV are offered — anything else falls back to CSV.
 *
 * An Excel (.xlsx) renderer existed briefly and was removed at the college's
 * request; `?format=xlsx` therefore lands on CSV, which Excel opens anyway,
 * rather than erroring for anyone holding an old link.
 */
...
export type DirectoryReport = {
  title: string;
  /** Provenance pairs — who, when, scope, filters, row count. */
...
export type DirectoryReportInput = {
  rows: DirectoryRow[];
...
/**
 * The report as data, before any file format touches it.
 *
 * CSV, Excel, and PDF all render from this one model, so the columns cannot
 * drift between formats — a student comparing the spreadsheet against the
 * printed copy must see the same figures in the same order.
 */
export function buildDirectoryReport({
  rows,
  filters,
  generatedBy,
  scopeNote,
  ...
  marksSummary,
}: DirectoryReportInput): DirectoryReport {
  ...
  return {
    title: "First-Year Portal — student export",
    meta,
...
/**
 * Headers shared by every format.
 *
 * These files contain student personal data — names, phone numbers, guardian
 * contacts — so no proxy or browser is allowed to keep a copy.
 */
...
export function directoryCsvResponse(input: DirectoryReportInput): NextResponse {
  const report = buildDirectoryReport(input);

  const rows: Array<Array<unknown>> = [
    [report.title],
    ...report.meta.map(([label, value]) => [label, value]),
    [],
    report.columns,
    ...report.data,
  ];
  ...
  return new NextResponse(toCsv(rows), {
    headers: fileHeaders(
      `${report.filenamePrefix}-${report.generatedAt.slice(0, 10)}.csv`,
